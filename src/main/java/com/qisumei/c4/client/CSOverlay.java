...
public class CSOverlay implements IGuiOverlay {
    ...
    public static void updateState(SyncMatchStatePacket packet) {
        ...
        bombPlanted = packet.bombPlanted; isRoundOver = packet.isRoundOver; roundWinner = packet.roundWinner; winReason = packet.winReason;
        terPlayers = packet.terPlayers; konterPlayers = packet.konterPlayers;
    }
    ...
    public void render(ForgeGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        Font font = mc.font;
        int centerX = screenWidth / 2; int y = 5;
        ...
        for (int i = 0; i < terPlayers.size(); i++) renderPlayerHead(guiGraphics, mc, terPlayers.get(i), terStartX + (i * 14), y + 5);

        int konterStartX = centerX + 70;
        for (int i = 0; i < konterPlayers.size(); i++) renderPlayerHead(guiGraphics, mc, konterPlayers.get(i), konterStartX + (i * 14), y + 5);

        if (isRoundOver && !roundWinner.isEmpty()) {
            int bannerY = 35;
            ...
            if (!winReason.isEmpty()) guiGraphics.drawString(font, winReason, centerX - font.width(winReason) / 2, bannerY + 26, 0xCCCCCC, true);
        }
    }
