...
public class SyncMatchStatePacket {
    public final int terScore, konterScore, timeLeftTicks, bombTimer;
    ...
    public SyncMatchStatePacket(FriendlyByteBuf buf) {
        ...
        this.bombPlanted = buf.readBoolean(); this.bombTimer = buf.readInt(); this.isRoundOver = buf.readBoolean();
        this.roundWinner = buf.readUtf(); this.winReason = buf.readUtf();
        this.terPlayers = new ArrayList<>();
        int tCount = buf.readInt(); 
        for (int i = 0; i < tCount; i++) this.terPlayers.add(new PlayerState(buf.readUUID(), buf.readUtf(), buf.readBoolean(), buf.readInt(), buf.readInt()));
        this.konterPlayers = new ArrayList<>();
        int ctCount = buf.readInt(); 
        for (int i = 0; i < ctCount; i++) this.konterPlayers.add(new PlayerState(buf.readUUID(), buf.readUtf(), buf.readBoolean(), buf.readInt(), buf.readInt()));
    }

    public void encode(FriendlyByteBuf buf) {
        buf.writeInt(terScore); buf.writeInt(konterScore); buf.writeInt(timeLeftTicks);
        buf.writeBoolean(bombPlanted); buf.writeInt(bombTimer); buf.writeBoolean(isRoundOver);
        buf.writeUtf(roundWinner); buf.writeUtf(winReason);
        buf.writeInt(terPlayers.size()); for (PlayerState p : terPlayers) { buf.writeUUID(p.uuid); buf.writeUtf(p.name); buf.writeBoolean(p.isAlive); buf.writeInt(p.kills); buf.writeInt(p.deaths); }
        buf.writeInt(konterPlayers.size()); for (PlayerState p : konterPlayers) { buf.writeUUID(p.uuid); buf.writeUtf(p.name); buf.writeBoolean(p.isAlive); buf.writeInt(p.kills); buf.writeInt(p.deaths); }
    }
    ...
    public static class PlayerState {
        ...
        public final int kills; public final int deaths;
        public PlayerState(UUID uuid, String name, boolean isAlive, int kills, int deaths) { 
